...
const legacyNormalizedRetryStateKey = "microsoft-normalized-v1/retry-origins-v1"
...
func (c *Client) bindRetryState(rt hosted.Storage, syncState func() error) error {
	...
	migrate := false
	for _, key := range []string{retryStateKey, legacyNormalizedRetryStateKey} {
		...
		migrate = migrate || key == legacyNormalizedRetryStateKey
	}
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	// Retain the predecessor key for rollback; never clear source histories.
	if migrate {
