...
func TestCanonicalDeploymentExamplesUseIndependentSelectorStanzas(t *testing.T) {
	tests := []struct {
		path   string
		header string
	}{
		{"microsoft_every_api.conf.example", "Microsoft"},
		{"../../docker-test/microsoft-collectors/config/hosted_runner.conf.example", "Microsoft"},
	}
	for _, tc := range tests {
		t.Run(filepath.ToSlash(tc.path), func(t *testing.T) {
			body, err := os.ReadFile(tc.path)
			if os.IsNotExist(err) {
				t.Skip("repository-wide example is not part of the portable plugin")
			}
			if err != nil {
				t.Fatal(err)
			}
			assertSelectorStanzaShape(t, string(body), tc.header)
		})
	}
}
