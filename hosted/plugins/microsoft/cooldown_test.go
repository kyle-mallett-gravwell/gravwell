...
func TestRetryStateMigrationPreservesStrongestEligibility(t *testing.T) {
	for _, mode := range []string{"disabled", "enabled"} {
		t.Run(mode, func(t *testing.T) {
			...
			old := map[string]map[string]retryEligibility{
				...
				legacyNormalizedRetryStateKey: {"https://raw.invalid": {Until: early}, "https://normalized.invalid": {Until: late}, "https://blocked.invalid": {Until: late}, "https://legacy-blocked.invalid": {Blocked: true}},
			}
			...
			if !merged["https://raw.invalid"].Until.Equal(late) || !merged["https://normalized.invalid"].Until.Equal(late) || !merged["https://blocked.invalid"].Blocked || !merged["https://legacy-blocked.invalid"].Blocked {
				t.Fatalf("eligibility weakened: %#v", merged)
			}
			b, err = rt.Get(legacyNormalizedRetryStateKey)
			if err != nil {
				t.Fatal(err)
			}
			var legacy map[string]retryEligibility
			if err := json.Unmarshal(b, &legacy); err != nil {
				t.Fatal(err)
			}
			if len(legacy) != len(old[legacyNormalizedRetryStateKey]) {
...
func TestRetryStateMigrationFailsClosed(t *testing.T) {
	for _, failure := range []string{"decode", "put", "sync"} {
		t.Run(failure, func(t *testing.T) {
			...
			if err := rt.Put(legacyNormalizedRetryStateKey, b); err != nil {
				t.Fatal(err)
			}
			rt.fail = failure == "put"
			...
			err = c.bindRetryState(rt, func() error {
				if failure == "sync" {
					return errors.New("synthetic migration sync failure")
				}
				return nil
			})
