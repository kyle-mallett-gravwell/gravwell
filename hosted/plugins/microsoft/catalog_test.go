...
func TestMicrosoftCollectorReleaseVersion(t *testing.T) {
	if Version != "1.3.2" {
		t.Fatalf("ingester release version=%q want=1.3.2", Version)
	}
...
func TestDefenderCloudDefaultTagMigrationKeepsSelectorStateKeysStable(t *testing.T) {
	dataset := datasets["defender-cloud-assessments"]
	if dataset.Name != "defender-cloud-assessments" {
		t.Fatalf("selector changed during tag migration: %q", dataset.Name)
	}
	...
	if got, want := stateKey("microsoft", dataset.Name, "subscription"), "microsoft/defender-cloud-assessments/subscription"; got != want {
		t.Fatalf("tag migration changed state key: got=%q want=%q", got, want)
	}
}
